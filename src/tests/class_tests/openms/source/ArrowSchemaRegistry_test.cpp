...
START_SECTION(PSMSchema column name constants)
{
  ...
  TEST_STRING_EQUAL(PSMSchema::HIT_INDEX, "hit_index")
...
START_SECTION(PSMSchema field types and nullability)
{
  ...
  TEST_EQUAL(s->field(19)->nullable(), true)
  // hit_index: int32, nullable (default) — positional column, distinct from
  // PeptideHit::getRank() which round-trips via the "rank" UserParam.
  TEST_EQUAL(s->field(20)->name(), "hit_index")
  TEST_EQUAL(s->field(20)->type()->id(), arrow::Type::INT32)
